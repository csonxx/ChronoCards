...
class BattleScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      ...
      child: BattleView(enemyId: enemyId),
    );
  }
}
...
class BattleView extends StatefulWidget {
  final String? enemyId;
  const BattleView({super.key, this.enemyId});
...
class _BattleViewState extends State<BattleView> with TickerProviderStateMixin {
  String? get enemyId => widget.enemyId;
  ...
  void didChangeDependencies() {
    ...
    if (provider.isVictory) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) _triggerVictorySequence(context, provider);
      });
    } else if (provider.isDefeat) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) _showDefeatDialog(context, provider);
      });
    }
  }
